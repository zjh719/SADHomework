...
// CodeMirror is the only global var we claim
    var CodeMirror = (function() {
        ...
        if (gecko) // Only for "$'"
            ...
            spanAffectsWrapping = function(str, i) {
                ...
                return /[~!#%&*)=+}\]|\"\.>,:;][({[<]|-[^\-?\.\u2010-\u201f\u2026]|\?[\w~`@#$%\^&*(_=+{[|><]|鈥\w~`@#$%\^&*(_=+{[><]/.test(str.slice(i - 1, i + 1));
            };
    ...
    function createIcon(name, options) {
        ...
        if (shortcut) {
            ...
            el.title = shortcut;
            el.title = el.title.replace('Cmd', '鈱�');
            if (isMac) {
                el.title = el.title.replace('Alt', '鈱�');
            }
        }
    ...
    function cmDrawImageUp(editor, url) {
        var cm = editor.codemirror;
        var stat = getState(cm);
        _replaceSelection(cm, stat.image, '![', '鍥剧墖璇存槑]('+ url +')');
        $('.cm_add_img').slideUp(300);
        $('.cm_dialog').fadeOut(300);
    }
    ...
    var toolbar = [
        '|',
        {name: 'headline', action: toggleHeadline,title:'鏍囬涓€锛圕trl+Alt+1锛�'},
        {name: 'heading', action: toggleHeading,title:'鏍囬浜岋紙Ctrl+Alt+2锛�'},
        {name: 'bold', action: toggleBold,title:'绮椾綋锛圕trl+B锛�'},
        {name: 'italic', action: toggleItalic,title:'鏂滀綋锛圕trl+I锛�'},
        '|',

        {name: 'quote-left', action: toggleBlockquote,title:'寮曠敤锛圕trl+鈥欙級'},
        {name: 'code', action: drawCode,title:'鎻掑叆浠ｇ爜鐗囷紙Ctrl+,锛�'},
        {name: 'list-ul', action: toggleUnOrderedList,title:'鏃犲簭鍒楄〃锛圕trl+L锛�'},
        {name: 'list-ol', action: toggleOrderedList,title:'鏈夊簭鍒楄〃锛圕trl+Alt+L锛�'},
        '|',

        {name: 'link', title:'娣诲姞閾炬帴锛圕trl+K锛�'},    //action: drawLink,
        {name: 'picture', title:'娣诲姞鍥剧墖锛圕trl+Alt+I锛�'},   //action: drawImage,
        '|',

        {name: 'reply', action: undo,title:'鎾ら€€锛圕trl+Z锛�'},
        {name: 'share-alt', action: redo,title:'鍓嶈繘锛圕trl+Shift+Z锛�'},
        '|',
        {name: 'info', action: 'http://ask.csdn.net/pages/markdown',title:'markdown璇硶鍙傝€�'},
        {name: 'preview', action: togglePreview,title:'棰勮'},
